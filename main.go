...
//	-i, --install          Install output to ~/.config/t-l3/sh-gen/{[name].comp.sh,[name].lazycomp.sh}.
//	-n, --name    <name>   Override the program name used in the completion script.
...
func run() error {
	...
	flag.BoolVar(&shouldInstall, "install", false, "Install output to ~/.config/t-l3/sh-gen/{[name].comp.sh,[name].lazycomp.sh}")
	flag.BoolVar(&shouldInstall, "i", shouldInstall, "See --install")
	flag.StringVar(&overrideName, "name", "", "Override the program `name` used in the completion script")
	flag.StringVar(&overrideName, "n", overrideName, "See --name `name`")
